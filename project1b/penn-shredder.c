...
void executeShell(int timeout) {
    ...
    alarm(0);                       // (1) cancel any previous alarm
    ...
    if (command != NULL) {
        ...
        if (childPid == 0) {
            // (optional but  recommended) avoid SIGINT race
            ...
            if (timeout > 0) {      // (2) start timer in parent
                alarm(timeout);
            }
            ...
            alarm(0);               // (3) child finished; cancel timer
            childPid = 0;           // (4) reset global
            free(command);
        }
    }
}
...
char *getCommandFromInput() {
    ...
    /* Ctrl-D with no text, then EOF immediately */
    if (n == 0) {
        exit(0);
    }
